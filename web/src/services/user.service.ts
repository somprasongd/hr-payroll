...
export const userService = {
  ...
  updateUserRole: async (id: string, role: 'admin' | 'hr'): Promise<User> => {
    return apiClient.patch<User>(`/admin/users/${id}`, { role });
  },

  resetUserPassword: async (id: string, data: ResetPasswordRequest): Promise<{ message: string }> => {
    return apiClient.post<{ message: string }>(`/admin/users/${id}/password-reset`, data);
  },
