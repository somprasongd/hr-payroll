...
import { DebtPrintTemplate } from './debt-print-template';
import { RepaymentReceiptPrintTemplate } from './repayment-receipt-print-template';

interface DebtPrintDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  debtId: string;
}

export function DebtPrintDialog({ open, onOpenChange, debtId }: DebtPrintDialogProps) {
  const t = useTranslations('Debt.print');
  const componentRef = useRef<HTMLDivElement>(null);
  
  const [loading, setLoading] = useState(false);
  const [debt, setDebt] = useState<DebtTxn | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const [debtData, profileData] = await Promise.all([
        debtService.getDebtTxn(debtId),
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Loan_Agreement_${debt?.employeeId}_${debt?.txnDate}`,
  });
        ...
        <div className="flex-1 overflow-auto bg-gray-100 p-4">
          <div className="max-w-[210mm] mx-auto bg-white shadow-lg min-h-[297mm]">
            {loading ? (
              ...
              debt.txnType === 'repayment' ? (
                <RepaymentReceiptPrintTemplate
                  ref={componentRef}
                  debt={debt}
