...
// GetPendingCycle returns the pending salary raise cycle for the tenant (if any)
...
// AddEmployeeToPendingCycle adds an employee to the pending salary raise cycle
func (r Repository) AddEmployeeToPendingCycle(ctx context.Context, tenant contextx.TenantInfo, employeeID uuid.UUID, actor uuid.UUID) error {
	...
	// Insert salary_raise_item with snapshot data from employee
	q := `
...
// RemoveEmployeeFromPendingCycle removes an employee from the pending salary raise cycle
func (r Repository) RemoveEmployeeFromPendingCycle(ctx context.Context, tenant contextx.TenantInfo, employeeID uuid.UUID) error {
	...
	q := `DELETE FROM salary_raise_item WHERE cycle_id = $1 AND employee_id = $2`
