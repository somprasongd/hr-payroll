package get

import (
	"context"
	"database/sql"

	"github.com/google/uuid"
	"go.uber.org/zap"

	"hrms/modules/payrollorgprofile/internal/dto"
	"hrms/modules/payrollorgprofile/internal/repository"
	"hrms/shared/common/contextx"
	"hrms/shared/common/errs"
	"hrms/shared/common/logger"
	"hrms/shared/common/mediator"
)

type Query struct {
	ID uuid.UUID
}

type Response struct {
	Profile dto.Profile `json:"profile"`
}

type Handler struct {
	repo repository.Repository
}

var _ mediator.RequestHandler[*Query, *Response] = (*Handler)(nil)

func NewHandler(repo repository.Repository) *Handler {
	return &Handler{repo: repo}
}

func (h *Handler) Handle(ctx context.Context, q *Query) (*Response, error) {
	if _, ok := contextx.TenantFromContext(ctx); !ok {
		return nil, errs.Unauthorized("missing tenant context")
	}

	rec, err := h.repo.Get(ctx, q.ID)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, errs.NotFound("profile not found")
		}
		logger.FromContext(ctx).Error("failed to get org profile", zap.Error(err), zap.String("id", q.ID.String()))
		return nil, errs.Internal("failed to get org profile")
	}

	return &Response{Profile: dto.FromRecord(*rec)}, nil
}
