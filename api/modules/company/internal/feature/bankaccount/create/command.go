...
func (h *commandHandler) Handle(ctx context.Context, cmd *Command) (*Response, error) {
	...
	item, err := h.repo.CreateBankAccount(ctx, cmd.BankID, cmd.BranchID, cmd.AccountNumber, cmd.AccountName, user.ID)
	if err != nil {
		return nil, errs.Internal("failed to create bank account")
	}
