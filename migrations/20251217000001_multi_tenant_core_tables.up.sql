...
-- Update payroll config guard to allow superadmin to create payroll config too
...
BEGIN
    ...
    IF v_creator_role NOT IN ('admin', 'superadmin') THEN
        RAISE EXCEPTION 'Access Denied: Only administrators or superadmin can create new payroll config. (User ID % is %)', NEW.created_by, v_creator_role;
    END IF;
