-- 1. Drop Manual Override Flags
ALTER TABLE payroll_run_item DROP COLUMN IF EXISTS is_manual_tax;
ALTER TABLE payroll_run_item DROP COLUMN IF EXISTS is_manual_pf;
ALTER TABLE payroll_run_item DROP COLUMN IF EXISTS is_manual_internet;
ALTER TABLE payroll_run_item DROP COLUMN IF EXISTS is_manual_water;
ALTER TABLE payroll_run_item DROP COLUMN IF EXISTS is_manual_electric;

-- 2. Revert function to previous state (standard calculation without manual checks)
...
BEGIN
  ...
  SELECT COALESCE(others_income, '[]'::jsonb), COALESCE(others_deduction, '[]'::jsonb)
  INTO v_others_income, v_others_deduction
