-- 1. Add columns
ALTER TABLE payroll_run_item ADD COLUMN IF NOT EXISTS employee_settings_snapshot JSONB DEFAULT '{}'::jsonb;
ALTER TABLE payroll_run ADD COLUMN IF NOT EXISTS payroll_config_id UUID REFERENCES payroll_config(id);


-- 2. Backfill employee_settings_snapshot
-- Disable guard trigger to allow updating 'approved' runs
ALTER TABLE payroll_run_item DISABLE TRIGGER tg_payroll_run_item_guard_biu;

DO $$
BEGIN
  UPDATE payroll_run_item pri
  SET employee_settings_snapshot = 
    CASE 
      -- Case 1: Pending run (use current employee settings)
      WHEN pr.status = 'pending' THEN 
        COALESCE((
          SELECT jsonb_build_object(
            'base_pay_amount', e.base_pay_amount,
            'sso_contribute', e.sso_contribute,
            'provident_fund_contribute', e.provident_fund_contribute,
            'withhold_tax', e.withhold_tax,
            'allow_housing', e.allow_housing,
            'allow_water', e.allow_water,
            'allow_electric', e.allow_electric,
            'allow_internet', e.allow_internet,
            'allow_doctor_fee', e.allow_doctor_fee,
            'allow_attendance_bonus_nolate', e.allow_attendance_bonus_nolate,
            'allow_attendance_bonus_noleave', e.allow_attendance_bonus_noleave
          )
          FROM employees e 
          WHERE e.id = pri.employee_id
        ), '{}'::jsonb)
      
      -- Case 2: Approved / Processed run (heuristic: amount > 0 means true)
      ELSE
        jsonb_build_object(
          'base_pay_amount', CASE WHEN pri.pt_hourly_rate > 0 THEN pri.pt_hourly_rate ELSE pri.salary_amount END,
          'sso_contribute', (pri.sso_month_amount > 0),
          'provident_fund_contribute', (pri.pf_month_amount > 0),
          'withhold_tax', (pri.tax_month_amount > 0),
          'allow_housing', (pri.housing_allowance > 0),
          'allow_water', (pri.water_amount > 0),
          'allow_electric', (pri.electric_amount > 0),
          'allow_internet', (pri.internet_amount > 0),
          'allow_doctor_fee', (pri.doctor_fee > 0),
          'allow_attendance_bonus_nolate', (pri.attendance_bonus_nolate > 0),
          'allow_attendance_bonus_noleave', (pri.attendance_bonus_noleave > 0)
        )
    END
  FROM payroll_run pr
  WHERE pri.run_id = pr.id
    AND (pri.employee_settings_snapshot IS NULL OR pri.employee_settings_snapshot = '{}'::jsonb);
END $$;

-- Re-enable guard trigger
ALTER TABLE payroll_run_item ENABLE TRIGGER tg_payroll_run_item_guard_biu;


-- 3. Backfill payroll_config_id for existing runs
-- Disable guard trigger to allow updating 'approved' runs
ALTER TABLE payroll_run DISABLE TRIGGER tg_payroll_run_guard_update;

DO $$
BEGIN
  -- Update approved/pending runs with the most "effective" config for their period
  UPDATE payroll_run pr
  SET payroll_config_id = (
    SELECT id
    FROM payroll_config pc
    WHERE pc.company_id = pr.company_id
      AND pc.effective_daterange @> pr.payroll_month_date
    ORDER BY lower(pc.effective_daterange) DESC, pc.version_no DESC
    LIMIT 1
  )
  WHERE pr.payroll_config_id IS NULL;
END $$;

-- Re-enable guard trigger
ALTER TABLE payroll_run ENABLE TRIGGER tg_payroll_run_guard_update;


-- 4. Update payroll_run_generate_items
...
-- 5. Update sync_payroll_on_config_change
...
DECLARE
  ...
  v_config_end DATE;
BEGIN
  -- ทำงานเฉพาะ config ที่ยัง active
  ...
  LOOP
    -- เช็คว่า Config นี้ "มีผล" กับงวดเดือนนี้หรือไม่
    IF NEW.effective_daterange @> r_run.payroll_month_date THEN
      
      -- [UPDATE] Sync Config ID to Payroll Run
      UPDATE payroll_run
      SET payroll_config_id = NEW.id
      WHERE id = r_run.id;

      -- ถ้างวดตรง -> คำนวณใหม่ทุกคน
      FOR r_item IN 
        SELECT employee_id 
...
-- 6. Update recalculate_payroll_item
