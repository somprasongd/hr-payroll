...
- **Query Parameters:**
  ...
  - `status`: (string) `pending`, `approved`, `all` (ไม่ส่ง = all)
...
- `409 Conflict`: มี worklog ของพนักงานเดียวกันในวันเดียวกันอยู่แล้ว (ยังไม่ถูกลบ)
...
ยกเลิกใบเบิกจ่ายที่ยังไม่จ่าย (status: `to_pay` เท่านั้น) และคืน worklog ให้เลือกใหม่ได้

- **Endpoint:** `DELETE /payouts/pt/{id}`

**Logic การทำงาน:**

1. ตรวจสอบว่า payout status เป็น `to_pay` และยังไม่ถูกลบ
2. Soft delete payout และรายการ worklog ที่ผูกไว้
3. คืนสถานะ worklog กลับเป็น `pending` เพื่อสามารถเลือกเข้ารายการอื่นได้
4. ตอบกลับ `204 No Content`

**Error Responses:**

- `400 Bad Request`: สถานะไม่ใช่ `to_pay`
- `404 Not Found`: ไม่พบรายการหรือถูกยกเลิกไปแล้ว
...
| `employeeNumber` | String     | รหัสพนักงาน (employee_number) |
| `employeeName`   | String     | ชื่อ-สกุลพนักงาน              |
| `tenureDays`     | Integer    | อายุงาน (วัน)                 |
| `currentSalary`  | Number     | เงินเดือนปัจจุบัน             |
| `currentSsoWage` | Number     | ฐานประกันสังคมปัจจุบัน        |
| `raisePercent`   | Number     | เปอร์เซ็นต์ที่ปรับเพิ่ม/ลด    |
| `raiseAmount`    | Number     | เงินปรับเพิ่ม/ลด              |
| `newSalary`      | Number     | เงินเดือนใหม่                 |
| `newSsoWage`     | Number     | ฐานประกันสังคมใหม่            |
| `stats.*`        | Object     | ค่า late/leave/ot ตามช่วงรอบ  |
  ...
  "data": [
    {
      ...
      "bonusYear": 2025,                // ปี ค.ศ. ของโบนัส (default = ปีปัจจุบัน)
...
- **Constraints / Business Rules:**
  ...
  - ระบบบันทึก `bonusYear` เป็นปี ค.ศ. (ส่งเองได้; ไม่ส่ง = ใช้ปีของ `payrollMonthDate`) เพื่อง่ายต่อการค้นหา/รายงาน
...
```json
{
  ...
  "bonusYear": 2025, // (optional) ปี ค.ศ. ของโบนัส; ไม่ส่ง = ดึงปีจาก payrollMonthDate
...
- ส่งกลับรายการพนักงานในรอบนั้น พร้อมสถิติ (late/leave/ot) เพื่อกรอกโบนัส
- เรียงผลลัพธ์ตาม `employeeNumber` จากน้อยไปมาก
...
- `photoId` มาจากพนักงาน ใช้แสดงรูปใน UI
...
```json
{
  ...
  "othersIncome": [{ "name": "ค่าคอมมิชชั่น", "value": 1000.0 }],
  "othersDeduction": [{ "name": "ค่าปรับ", "value": 500.0 }],
  "incomeTotal": 35000.0,
  ...
  "deductionTotal": 4650.0,
  "netPay": 30350.0,
  "status": "pending"
}
```

ฟิลด์ `employeeTypeName`, `departmentName`, `positionName`, `bankName`, `bankAccount` เป็น snapshot จาก `payroll_run_item` เพื่อใช้พิมพ์/แสดงบนสลิป; ค่าที่เป็น JSONB (`othersIncome`, `othersDeduction`, `loanRepayments`) ถูกแปลงเป็น array of objects ใน response
...
```json
{
  ...
  "othersIncome": [
    // เพิ่มรายได้อื่นๆ
    { "name": "ค่าคอมมิชชั่น", "value": 5000.0 }
  ],
  "othersDeduction": [
    // หักเพิ่มเติม
    { "name": "หักค่าปรับ", "value": 300.0 }
  ],
...
|               | `ptHoursWorked`    | `pt_hours_worked` (part_time เท่านั้น)     |
|               | `ptHourlyRate`     | `pt_hourly_rate` (part_time เท่านั้น)      |
|               | `ot`               | `ot_amount`                                |
|               | `bonus`            | `bonus_amount`                             |
...
|               | `othersDeduction`  | `others_deduction` (JSONB)                 |
...
กลุ่ม API สำหรับดูประวัติการใช้งานระบบ (Audit Trail)

### 18.1 List Activity Logs

ดึงรายการ Activity Logs พร้อม Pagination และ Filter
...
- **Query Parameters:**
  ...
  - `entity` (e.g., USER, EMPLOYEE, DEPARTMENT)
  - `fromDate` (YYYY-MM-DD)
  - `toDate` (YYYY-MM-DD)
...
```json
{
  "data": [
    {
      "id": "019347f5-1234-7d69-9f3a-1c3d4e5f9999",
      "userId": "019347e8-94ca-7d69-9f3a-1c3d4e5f6789",
      "userName": "admin",
      "action": "CREATE",
      "entity": "EMPLOYEE",
      "entityId": "019347f5-abcd-7d69-9f3a-567890abcdef",
      "details": {
        "code": "EMP001",
        "firstName": "John",
        "lastName": "Doe"
...
| ------------------ | --------------------------------------------- | ----------------- | ------------ |
| `data[].id`        | รหัส Log (UUIDv7)                             | UUID              | **Yes**      |
| `data[].userId`    | รหัสผู้ดำเนินการ                              | UUID              | **Yes**      |
| `data[].userName`  | ชื่อผู้ใช้งานที่ดำเนินการ                     | String            | **Yes**      |
| `data[].action`    | ประเภทการกระทำ (CREATE, UPDATE, DELETE, etc.) | String            | **Yes**      |
| `data[].entity`    | ชื่อ Entity ที่ถูกกระทำ                       | String            | **Yes**      |
| `data[].entityId`  | รหัสของ Entity ที่ถูกกระทำ                    | String            | **Yes**      |
| `data[].details`   | รายละเอียดเพิ่มเติม (JSON Object หรือ null)   | Object            | No           |
| `data[].createdAt` | เวลาที่บันทึก Log                             | String            | **Yes**      |

---

### 18.2 Get Latest Activity Logs

ดึง Activity Logs ล่าสุด (สำหรับ Dashboard)

- **Endpoint:** `GET /admin/activity-logs/latest`
- **Access:** Admin
- **Query Parameters:** `limit` (default: 5)
- **Response:** เหมือน 18.1

---

### 18.3 Get Filter Options

ดึงค่า Action และ Entity ที่มีอยู่ใน Activity Logs สำหรับใช้เป็น Filter Dropdown

- **Endpoint:** `GET /admin/activity-logs/filter-options`
- **Access:** Admin
...
```json
{
  ...
  "entities": ["DEPARTMENT", "EMPLOYEE", "PAYROLL_ORG_PROFILE", "USER"]
}
```
...
| **ชื่อ (Name)** | **คำอธิบาย (Description)**          | **ประเภท (Type)** | **Required** |
| --------------- | ----------------------------------- | ----------------- | ------------ |
| `actions`       | รายการ Action ที่มีใน Activity Logs | Array of String   | **Yes**      |
| `entities`      | รายการ Entity ที่มีใน Activity Logs | Array of String   | **Yes**      |
