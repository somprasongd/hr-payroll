...
- **Admin:** ทำได้ทุกอย่าง (Create, Read, Update, Delete)
- **Staff:** ทำได้เพียง เพิ่ม (Create), อ่าน (Read), และ แก้ไข (Update) **ไม่สามารถลบได้**
...
| **HTTP Status** | **Title** | **Description** |
| --- | --- | --- |
| **403** | Forbidden | ผู้ใช้งานเป็น Staff (ไม่มีสิทธิ์ลบ) |
| **404** | Not Found | ไม่พบพนักงาน |
...
- **Staff:** **ดู (Read) เท่านั้น**

### 7.1 Get Employee Accumulations

ดึงยอดเงินสะสมของพนักงาน (Staff ดูได้เพื่อตอบคำถามพนักงาน แต่แก้ไขไม่ได้)
...
| **403** | Forbidden | ผู้ใช้งานเป็น Staff (ไม่มีสิทธิ์แก้ไข) |
| **404** | Not Found | ไม่พบพนักงาน |
...
- **Staff:** สร้างรอบ (Draft), แก้ไขตัวเลขรายคน, ดูข้อมูล
...
- **Access:** Admin, Staff (Staff แก้ Status ไม่ได้)
...
- **Staff:** ห้ามส่ง field `status` (หรือส่งได้แค่ `pending`)
...
- `403 Forbidden`: Staff พยายามเปลี่ยนสถานะเป็น Approved
...
- **Staff:** สร้างรอบ (Draft), กรอกยอดเงินรายคน, ดูข้อมูล
...
- `400 Bad Request`: วันที่ผิด, หรือเดือนนี้มีรอบโบนัสอยู่แล้ว (ถ้า Business บังคับ 1 รอบต่อเดือน)
...
- **Staff:** ดูรายการ, สร้างรายการขอเบิก, แก้ไข/ลบ (เฉพาะสถานะ `pending`)
...
1. รับ `advanceDate` มาคำนวณหา `payrollMonthDate` (วันแรกของเดือนนั้น)
2. ตรวจสอบ Database Constraint (`salary_advance_date_in_month_ck`): วันที่เบิกต้องอยู่ในเดือนเดียวกับงวด Payroll
...
- **Staff:** สร้างคำขอเงินกู้ (Draft), ดูรายการ, บันทึกการคืนเงิน (Draft)
...
| **403** | Forbidden | ผู้ใช้งานเป็น Staff |
...
- **Access:** Admin (รับเงิน), Staff (บันทึกช่วย)
...
```json
{
  ...
  "employeeTypes": [
    { "id": "019a4...", "code": "ft", "name": "พนักงานประจำ" },
    { "id": "019a5...", "code": "pt", "name": "พนักงานชั่วคราว" }
  ],
  "idDocumentTypes": [
...
- **Staff:** สร้างงวด (Draft), ตรวจสอบรายการ, แก้ไขยอดรายคน (Prepare Data)
...
- **Access:** Admin Only (สำหรับการ Approve), Staff (แก้ Note/Date)
...
- **Access:** Admin, Staff (Admin/Staff ดูของทุกคน, User ดูของตัวเองผ่าน API อื่น)
- **Params:** `id` (UUIDv7 ของ Item)
